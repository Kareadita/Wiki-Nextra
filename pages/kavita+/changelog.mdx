...
#### Upcoming (v0.9.1)
- Added the ability to Scrobble with Hardcover, MAL, and MangaBaka
- Switched to OAuth for connection with Discord and Scrobble Providers - Kavita can now maintain freshness of tokens for scrobbling
- Full metadata from MangaBaka and Hardcover (Books/LNs)
- Ability to pin a library to a Metadata Provider
- Complete expansion/overhaul of License Management: Change Email, Renew, Cancel all from Kavita instance
- Ability to grab volume metadata and covers from Hardcover/MangaBaka (CBR already supported).
- Ability to write Series name to fixup constraints (ie - -> :)
- Support for different editions from MangaBaka (count difference (Omnibus) and different covers/summary)
- Ability for non-admin's to view external recommendations (Age Restrictions apply)
- Enhanced recommendations from MangaBaka based on other users in addition to existing tag comparison
- Kavita+ will now email you a month before charge for yearly subscriptions
- and more (still WIP)

#### v0.9.0.2
- Deployed a partial port to MangaBaka to avoid severe downtime issues with AniList

##### v0.8.8
- Fixed a backend bug with matching on CBR links

##### v0.8.7
- Added ability to download metadata, ratings, and reviews for Series and Issues for Comics (ComicBookRoundup - 16K Series, 112K Issues atm of release). This includes weekly releases.
