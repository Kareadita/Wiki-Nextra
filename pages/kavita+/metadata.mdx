...
0. Field mapping is applied to Genre/Tag list. If `Source Removed` is selected, the item will be removed from the list.
1. Both Genres and Tags will pass through Blacklist list and any blacklisted items will be removed.
2. Tags will pass through the whitelist if applicable and only whitelisted strings will be kept.
3. If the setting is enabled, the process will continue. If not, these processed genres/tags will be used in the `Age Rating Mapping` step.
...
## People
People by default will not be removed from a metadata match if there is no match. Kavita however will remove people from a bad match perviously.
