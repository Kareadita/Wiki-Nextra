...
A. Kavita does not yet offer mobile apps, however, you can "Add to Home screen" our website, which will create an icon on your mobile device and launch the site in fullscreen mode.

Check out the growing list of [3rd Party Clients](../../guides/3rdparty/aidoku) and [OPDS Capable Clients](../../guides/features/opds/#opds-capable-clients)
...
A. By default, Kavita will collect stats on your installation. This will run after 24 hours to give you time to opt-out. You can opt-out at any time by turning off "Send Data" from the Admin Dashboard. All data is anonymized and contains no information about your filenames or IP. The Kavita team actively uses this data to help design the UX and plan enhancements. If you choose to remain opted-in, thank you. It helps in the design and planning effort. You can view the code at any time [here](https://github.com/Kareadita/KavitaStats). Here are the [records](https://github.com/Kareadita/KavitaStats/tree/main/KavitaStats/Entities/V3) from our stats database:
...
#### Q. How do I restore the DB?
A. By default, Kavita stores backups in `config/backups`. To restore from an older version:
- Stop Kavita
- Copy everything in the backup zip into the `config/` directory
- Start Kavita

#### Q. I moved my files from one drive to another, how can I tell Kavita about this.
A. Kavita can make this update without data loss. If docker, just change the mount point. If native,
- Open Tasks in Settings and perform a backup
- Add the new library path to your Library
- Perform a scan library
- Remove old library path
- Open a Series and validate the FolderPath aligns with expectations.
