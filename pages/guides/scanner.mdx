...
- Image files named `cover`/`!cover` or `folder` (eg. `cover.jpg`, `folder.png`) are treated as a cover image, not a page, and are excluded from parsing/reading. Files containing `backcover` are not treated as a cover.
