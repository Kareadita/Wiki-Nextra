...
<Callout type="warning">
    We no longer recommend using komf with kavita. The core app has not been updated in months and has issues that have not been addressed. The extension has not worked properly since it's launch where the icons have not appeared in order to properly configure it. Until recently the deprecated userscript was working and has since been having similar issues. We do not currently know of any free alternative for adding metadata directly into kavita, if you do know of one or have one please let us know so we can look into it, but Kavita+ is getting a major facelift in 0.10.0 with Mangabaka and Hardcover support added on top of us getting data from MAL, AL, and ComicBookRoundup. We will continue to leave this wiki page open as komf does still function in some capacity but it is limited in how it works with kavita.
</Callout>
...
kavita:
  ...
  apiKey: "YOUR_AUTH_KEY_HERE!" #or env:KOMF_KAVITA_API_KEY

database:
  file: ./database.sqlite # database file location.
```

Fill out your `baseUri` to point to your Kavita install URL and put in your [Kavita Auth key](../user-settings/3rdpartycilents.mdx#api-key). 

<Callout type="info">
  The Auth key you use will be the user that Komf is logging in as. Make sure that the user has the correct permissions in Kavita to do what you're asking of it.
</Callout>

<Callout type="warning">
    If you're connecting to Kavita over https, you'll also need to connect to Komf over https.
</Callout>
