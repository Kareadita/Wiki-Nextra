...
### Notes
ComicTagger is best used for "western comics". You'll need an account with [ComicVine](https://comicvine.gamespot.com/api/) or [Metron](https://metron.cloud/accounts/signup/) in order to use the application. 

While ComicVine does have some manga in it's database, it's no where as much as comics. If the series your trying to find is not found in the database then fetching metadata from the service won't work.
