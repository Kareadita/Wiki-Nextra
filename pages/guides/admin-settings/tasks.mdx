...
Recurring tasks have the ablity to be customize their run time with cron jobs. [Check your cron](https://www.crontab.guru) syntax first before saving. 
...
#### Kavita Backup
You can set the frequency that you want Kavita to backup it's database and files. 
...
Kavita has a handful of processes that it need to clean up eventually. This does tasks like reclaiming disk space and clearing out temp files.

## Ad-hoc Tasks

These are one off tasks that you are able to be ran at any time. They do not have a schedule. 
...
For Kavita+ users to clear out the K+ cache. This might be needed if a series matches to the wrong series and keeps showing back up.

#### Bust Locale Cache

This can fix issues with localizaion strings not showing correctly after an update.
...
Compiles all log files into a zip and downloads it. Useful if your remote to your instance and need logs.

#### Analyze Files

Legacy scan option that starts a long-running task which will analyze files to generate extension and size. For users that had Kavita before 0.7.0 installed. 
