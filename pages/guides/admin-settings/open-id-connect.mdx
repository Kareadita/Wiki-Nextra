...
- **Roles claim**: Set the roles claim to `groups` when using the `Sync user settings` feature. This ensures that user group memberships are properly synchronized with Kavita's role-based access control.
...
- **Scopes**: Ensure the following scopes are configured in your Authelia client to provide the required user information to Kavita:
  - `openid`
  - `email`
  - `profile`
  - `groups`
