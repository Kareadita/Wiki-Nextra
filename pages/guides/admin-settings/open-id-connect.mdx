...
Kavita supports authentication and user management via any OpenID Connect-compliant provider.
...
You may need to configure the following URLs in your OIDC provider:
...
The audience (`aud`) claim in your ID token **must match** the Client ID you have configured in Kavita.
...
- Age rating: `age-rating-<Rating>` (e.g., `age-rating-KidsToAdults`)
- To allow unknown ratings: `age-rating-include-unknowns`
...
- **User Client Role Mapper**: Ensure user roles are exposed in the ID token under either the `role` or `roles` claim.
