...
e.g. If you have the prefix `kavita-` configured, your age restricting role would be `kavita-age-restriction-KidsToAdults`
...
<Callout type="warning">
    The default is `http://schemas.microsoft.com/ws/2008/06/identity/claims/role` and should be used when the roles are under `roles` Using `roles`will **not** work.
</Callout>
...
- **Roles claim**: Ensure the roles claim is set to `groups` when using the sync users feature
