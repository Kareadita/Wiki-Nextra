...
- **Authority**: While Kavita refers to authority as the url that it gets the provider information from Authentik refers to this URL as `OpenID Configuration Issuer`.
...
To use the more conventional method of pulling `Roles` from a `Group` in Authentik and using OIDC-Managed permissions, you will need to setup a `Property Mapping` within Authentik. Once a user group (such as `kavita-user`) is created, assign roles to this group for authentication and library access. 
For example, a user group with the following roles will be allowed most non-admin access and access to 4 libraries. Unicode library names should use them as-is and spaces should also be used as-is.
```
kavita-login
kavita-promote
kavita-bookmark
kavita-change password
kavita-download
kavita-library-単行本
kavita-library-books
kavita-library-manga
kavita-library-manga english
```
...
```
- Optionally, create another `Scope Mapping` Property Mapping to always return a users email address as verified so they will also be verified within Kavita:
```
return {
    "email_verified": True
}
```
- **Authentik Provider Settings**: In addition to the typical `email`, `openid` and `profile` scopes, ensure `offline_access` and the Property Mappings you created above are included.
Then in Kavita, configure the following:
- **Provision accounts**: Ensure this is toggled on to allow automatic user creation via OIDC login, if intended.
...
- **Roles Claim**: Ensure the roles claim is set to `roles`.
