---
title: 'Reading Lists'
---
import { Steps } from 'nextra/components'


# Reading Lists

Reading Lists are an ordered set of chapters that can be created by any user and promoted (made visible to all users on the server) by admins. Like Collections, they can have a cover image and summary. 

The main feature of a reading list is the order. Reading order is enforced at the chapter level. When reading said list, the readers are capable of moving between different types of readers. So the list could start with Manga then move into epub reader then to pdf and back to Manga without ever leaving the reader. 

The list order can be changed by using drag and drop or clicking Order Numbers and shifting the order number directly. 

![lists](../../../public/guides/features/readinglists.png)

![detail](../../../public/guides/features/readinglist_detail.png)

## Creating a Reading List via CBL
Kavita supports the creation of Reading Lists via CBL files. From the side menu, click the ... menu next to `Home` and select `Import CBL`.

<Steps>

## Step 1 

This will bring up the initial screen showing the wizard-like import. From here, the user can select multiple CBL files. 
![First%20Step](../../../public/guides/features/import_readinglist_step1.png) 

## Step 2

Here you see that I have uploaded multiple files. I can remove which ones I might not want to move to the next step.
![2nd%20Step](../../../public/guides/features/import_readinglist_step2.png)

Upon hitting next, you can see that some of these failed the validation step. Validation is a lightweight step meant to quickly cull errors, like invalid CBL formats, missing access to series/libraries (via rating or library restrictions), etc. Those that have failed will not be moved to the next step. You do not need to manually remove them. 

## Step 3
![3rd%20Step](../../../public/guides/features/import_readinglist_step3.png)

The next step is a dry run. Validation step can't catch everything that might happen during reading list creation, so the dry run pretends to create the Reading List without saving it to the DB. Here you will see more errors around missing issues/volumes, etc. You will not be able to move forward unless there is at least partial or full success. Failures will prevent import as that implies everything in the list failed.

## Step 4

![4th%20Step](../../../public/guides/features/import_readinglist_step4.png)

Once you're done, your reading list should appear in your library and you can restart the flow from the UI or close it and view your reading list. 

## Step 5

![Final%20Step](../../../public/guides/features/import_readinglist_step5.png)


</Steps>